...
suspend fun extractColorsFromUrl(
    ...
    return withContext(Dispatchers.IO) {
        try {
            // Load image as bitmap (must disable hardware bitmaps for Palette)
            val request = ImageRequest.Builder(context)
                .data(imageUrl)
                .allowHardware(false) // Required for Palette extraction
                .size(300, 169) // Small size for faster processing (16:9 ratio)
                .build()
            ...
            if (result is SuccessResult) {
                val bitmap = (result.image as? BitmapDrawable)?.bitmap
                    ?: return@withContext DynamicColors.Default
            ...
            // Log error in debug, return defaults
            DynamicColors.Default
        }
    }
}
...
fun rememberDynamicColors(imageUrl: String?): DynamicColors {
    ...
    // Get ImageLoader from Coil singleton
    val imageLoader = remember { ImageLoader(context) }
    
    LaunchedEffect(imageUrl) {
        colors = if (imageUrl != null) {
            extractColorsFromUrl(context, imageUrl, imageLoader)
        } else {
            DynamicColors.Default
        }
    }
...
fun rememberGradientColors(imageUrl: String?): GradientColors {
    ...
    LaunchedEffect(imageUrl) {
        colors = if (imageUrl != null) {
            extractGradientColorsFromUrl(context, imageUrl, imageLoader)
        } else {
...
private suspend fun extractGradientColorsFromUrl(
    ...
    return withContext(Dispatchers.IO) {
        try {
            ...
            if (result is SuccessResult) {
                val bitmap = (result.image as? BitmapDrawable)?.bitmap
                    ?: return@withContext GradientColors.Default
