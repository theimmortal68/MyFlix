 ...
 */
fun buildHomeDialogItems(item: JellyfinItem, actions: HomeDialogActions,): List<DialogItemEntry> = buildList {
 ...
 * @return List of DialogItemEntry for the dialog
 */
fun buildDetailDialogItems(item: JellyfinItem, actions: DetailDialogActions,): List<DialogItemEntry> = buildList {
    val isWatched = item.userData?.played == true
    val isFavorite = item.userData?.isFavorite == true
    val hasProgress = item.progressPercent > 0f && item.progressPercent < 1f
